...
class NotIntoStackTrace : BaseNetworkTestFactory() {
    @Get("change")
    @Test
    fun getNotIntoStackTrace() {
        ...
        val network = BaseNetwork<Array<UserJson>>(scope = CoroutineScope(Dispatchers.IO), "bla bla bla")
        network.call(
            ctx!!,
        ...
        network.data.observe(provideLifecycleOwner()) {
            when (it.status) {
                ...
                Status.ERROR -> assert(it.message?.contains("Class not present into stack trace") == true)
                Status.SUCCESS -> {
                }
            }
        }
